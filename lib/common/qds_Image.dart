...
Widget QdsImage(String url, double width, double height,
    {BoxFit fit = BoxFit.cover, String imagePlaceholder = baseConfig.defaultPlaceholder}) {
  if(url.isEmpty){
    return imagePlaceholder.isEmpty
        ? Container(
        color: ColorPlate.themeBgColor,
        child: const Icon(Icons.error,color: Colors.black,), width: width, height: height)
        : Image.asset(imagePlaceholder.mine, width: width, height: height, fit: fit);
  }
...
Widget QdsImageCorner(String url, double width, double height, double corner,
    {BoxFit fit = BoxFit.cover, String imagePlaceholder = baseConfig.defaultPlaceholder}) {
  return ClipRRect(
      child: QdsImage(url, width, height,fit: fit,imagePlaceholder: imagePlaceholder),
