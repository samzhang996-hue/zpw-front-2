...
class ZpwBuyEngin {
  late StreamSubscription<List<PurchaseDetails>> _zpwSubscription;
  late InAppPurchase _zpwInAppPurchase;
  late List<ProductDetails> _zpwProducts; //内购的商品对象集合
  ...
  void initializeInAppPurchase() {
    ...
    _zpwInAppPurchase = InAppPurchase.instance;

    //监听购买的事件
    final Stream<List<PurchaseDetails>> purchaseUpdated =
        _zpwInAppPurchase.purchaseStream;
    _zpwSubscription = purchaseUpdated.listen((purchaseDetailsList) {
      _listenToPurchaseUpdated(purchaseDetailsList);
    }, onDone: () {
      _zpwSubscription.cancel();
    }, onError: (error) {
      EasyLoading.dismiss();
      error.printError();
    });
  ...
  Future<void> resumePurchase() async {
    zpwIsPay = false;
    EasyLoading.show();
    final bool available = await _zpwInAppPurchase.isAvailable();
    if (!available) {
      if (_zpwVipLogic.click) {
        ZpwHandleTool.showAppToastText("无法连接到商店");
      }
    ...
    try {
      _zpwInAppPurchase.restorePurchases();
    } catch (e) {
      EasyLoading.dismiss();
      ZpwLog.d("$e");
    }
  ...
  void buyProduct(String productId) async {
    ...
    // }
    List<String> _zpwOutProducts = [productId];
    ...
    print("连接成功-开始查询全部商品");
    List<String> _zpwKIds = _zpwOutProducts;

    final ProductDetailsResponse response =
        await _zpwInAppPurchase.queryProductDetails(_zpwKIds.toSet());
    ...
    if (response.notFoundIDs.isNotEmpty) {
      EasyLoading.dismiss();
      if (_zpwVipLogic.click) {
        ZpwHandleTool.showAppToastText("未查询到商品订单");
      }
      //
      // print("无法找到指定的商品");
  ...
  void startPurchase(String productId) async {
    ...
    if (_zpwProducts != null && _zpwProducts.isNotEmpty) {
      ...
      try {
        ...
        _zpwInAppPurchase.buyConsumable(
  ...
  ProductDetails _getProduct(String productId) {
    return _zpwProducts.firstWhere((product) => product.id == productId);
  }
    ...
    for (PurchaseDetails purchase in purchaseDetailsList) {
      ...
      if (purchase.status == PurchaseStatus.pending) {
        ...
        if (Platform.isAndroid) {
          ...
          var appstoreDetail = purchase as AppStorePurchaseDetails;
          ZpwLog.d("sta----rl--${purchase.pendingCompletePurchase}");
          if (!("${appstoreDetail.purchaseID}" == zpwId &&
              zpwReData ==
                  appstoreDetail.verificationData.serverVerificationData)) {
            if (zpwIsPay == false) {
              _zpwVipLogic.restoreIosPay(
                  appstoreDetail.verificationData.serverVerificationData,
                  "${appstoreDetail.purchaseID}");
              zpwId = "${appstoreDetail.purchaseID}";
              zpwReData = appstoreDetail.verificationData.serverVerificationData;
  ...
  void _handleCancel(PurchaseDetails purchase) {
    _zpwInAppPurchase.completePurchase(purchase);
  }

  /// Android支付成功的校验
  void checkAndroidPayInfo(GooglePlayPurchaseDetails googleDetail) async {
    _zpwInAppPurchase.completePurchase(googleDetail);
  ...
  void checkApplePayInfo(AppStorePurchaseDetails appstoreDetail) async {
    _zpwInAppPurchase.completePurchase(appstoreDetail);
